...
namespace Repository.Repositories
{
    public abstract class RepositoryBase<T> : IRepositoryBase<T> where T : class
    {
        ...
        public IQueryable<T> FindAll(bool trackChanges) => !trackChanges ? _companyEmployeeDbContext.Set<T>().AsNoTracking() : _companyEmployeeDbContext.Set<T>();

        public IQueryable<T> FindByCondition(Expression<Func<T, bool>> expression, bool trackChanges) => 
            !trackChanges ? _companyEmployeeDbContext.Set<T>().Where(expression).AsNoTracking() : _companyEmployeeDbContext.Set<T>().Where(expression);
